...
```
python -m scripts.paper_smoke --yes > /tmp/paper_smoke.out 2>&1
rc=$?
cat /tmp/paper_smoke.out
echo "exit=$rc"
```

**Why not `… | tee … && echo $?`:** `python | tee` makes `$?` return
the exit code of `tee`, not Python. The bash-specific
`${PIPESTATUS[0]}` trick recovers Python's code, but on zsh (the
default shell on modern macOS) `PIPESTATUS` is 1-indexed, so the same
syntax silently returns the empty string. The redirect-then-cat pattern
above avoids the pipe entirely and works identically in bash and zsh.
