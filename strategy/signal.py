...
def evaluate_signal(frame: BarFrame, cfg: StrategyConfig) -> Signal | None:
    """Decide whether to enter on ``frame.symbol`` right now.

    Returns a :class:`Signal` on a BUY setup, or ``None`` otherwise.
    Pure — no exceptions for ordinary cases.
    """
    if (
